...
class SecurePRNG:
    ...
    def generate(self, n_bytes):
        ...
        while len(output) < n_bytes:
            ...
            output += block
            self.state = hashlib.sha256(self.state).digest()  # rollback resistance
...
class Mallory:
    ...
    def intercept(self, sender, recipient, payload):
        ...
        if isinstance(payload, bytes):
            ...
            print("[MALLORY] Decrypted message:", plaintext.decode())

            # Modify message
            modified = plaintext.replace(b"9pm", b"3am")

            # Re-encrypt for Bob
            new_payload = xor_crypt(modified, self.bob_prng)
...
def main():
    ...
    print(" [Status]: Shared Secret computed successfully.")

    print_step("Step 3: Secure Message Transmission")

    # 🔥 CHANGE ONLY THIS MESSAGE IF YOU WANT
    message = b"I will submit the lab before midnight."
    ...
    if b"3am" in final_message:
        print("\n[DANGER] MITM SUCCESS: Mallory decrypted and modified the message.")
